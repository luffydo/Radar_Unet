...
class CRDataset(data.Dataset):
    ...
    def __init__(self, data_dir, dataset, config_dict, split, is_random_chirp=False, subset=None, noise_channel=False):
        ...
        for seq_id, data_file in enumerate(tqdm(self.data_files)):
            # if seq_id > 1:
            #     continue
            data_file_path = os.path.join(data_dir, split_folder, data_file)                   #data_dir pkl/
    ...
    def __getitem__(self, index):
        ...
        try:
            ...
            if radar_configs['data_type'] == 'ROD2021':
                ...
                radar_npy_win = np.zeros((self.n_chirps, ramap_rsize, ramap_asize, 2),
                                            dtype=np.float32)                                                                      # self.win_size删除                                                              
                                                        #for循环删除
                for cid, c in enumerate(chirp_id):
                    npy_path = radar_paths[data_id][cid]                                                                  #frameid改成data_id
                    radar_npy_win[cid, :, :, :] = np.load(npy_path)                                                      #idx去掉
                data_dict['image_paths'].append(image_paths[data_id])                                                       #frameid改成data_id
                
            else:
                raise NotImplementedError

            data_dict['start_frame'] = data_id
            data_dict['end_frame'] = data_id 
            ...
            with open(os.path.join('./tmp', log_name), 'w') as f_log:
                f_log.write('npy path: ' + radar_paths[data_id][chirp_id] + \
                            '\nframe indices: %d:%d:%d' % (data_id, data_id, self.step))
            return data_dict

        # Dataloader for MNet
        if 'mnet_cfg' in self.config_dict['model_cfg']:
            radar_npy_win = np.transpose(radar_npy_win, (0, 3, 1, 2))
            assert radar_npy_win.shape == (
                self.n_chirps, 2, radar_configs['ramap_rsize'], radar_configs['ramap_asize'])
        else:
            radar_npy_win = np.transpose(radar_npy_win, (0, 3, 1, 2))
            assert radar_npy_win.shape == (2, radar_configs['ramap_rsize'], radar_configs['ramap_asize'])

        data_dict['radar_data'] = radar_npy_win

        # Load annotations
        if len(self.confmaps) != 0:
            confmap_gt = this_seq_confmap[data_id:data_id+1]
            confmap_gt = np.transpose(confmap_gt, (1, 0, 2, 3))
            confmap_gt = confmap_gt.reshape(4, 128, 128)
            obj_info = this_seq_obj_info[data_id:data_id]
            if self.noise_channel:
                assert confmap_gt.shape == \
                       (self.n_class + 1, radar_configs['ramap_rsize'], radar_configs['ramap_asize'])
            else:
                confmap_gt = confmap_gt[:self.n_class]
                assert confmap_gt.shape == \
                       (self.n_class, radar_configs['ramap_rsize'], radar_configs['ramap_asize'])
