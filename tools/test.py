...
def parse_args():
    ...
    parser.add_argument('--checkpoint', default='/media/zdk/SYQ/Radar_detection/RODNet-master/checkpoints/rodnet-unet-mnet-20221209-233131/epoch_26_final.pkl', type=str, help='path to the saved trained model')
...
if __name__ == "__main__":
    ...
    if not args.demo:
        ...
        seq_names = sorted(os.listdir(os.path.join('/media/zdk/SYQ/Radar_detection/RODNet-master/data/pkl/val')))
    print(seq_names)
    ...
    for subset in seq_names:
        ...
        if not args.demo:
            ...
            crdata_test = CRDataset(data_dir=args.data_dir, dataset=dataset, config_dict=config_dict, split='val',
                                    noise_channel=args.use_noise_channel, subset=None, is_random_chirp=False)
        ...
        for iter, data_dict in enumerate(dataloader):
            ...
            res_final = post_process_single_frame(confmap_pred, dataset, config_dict)                       #是否要降维，把batch qudiao
            ...
            if image_paths is not None:
                ...
                if confmap_gt is not None:
                    ...
                    visualize_test_img(fig_name, img_path, radar_input, confmap_pred,confmap_gt[0, :, :, :], res_final,          
                                        dataset, sybl=sybl)                                                           #res_final_0变成res_final，confmap_pred_0变成confmap_pred
                else:
                    visualize_test_img_wo_gt(fig_name, img_path, radar_input, confmap_pred, res_final,
                                                dataset, sybl=sybl)                                                          #res_final_0变成res_final，，confmap_pred_0变成confmap_pred
