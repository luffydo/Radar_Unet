...
def parse_args():
    ...
    parser.add_argument('--resume_from', type=str, default='/home/zdk/RODNet-master/checkpoints/rodnet-unet-mnet-20221213-141521/epoch_final.pkl', help='path to the trained model')
...
if __name__ == "__main__":
    ...
    for epoch in range(epoch_start, n_epoch):
        ...
        for iter, data_dict in enumerate(dataloader):
            ...
            # confmap_gt = confmap_gt.astype(np.float32)
            ...
            if iter % config_dict['train_cfg']['log_step'] == 0:
                ...
                writer.add_scalar('param/param_lr', optimizer.param_groups[0]['lr'], iter_count)
                ...
                chirp_amp_curr = chirp_amp(data.numpy()[0, 3, :,  :, :], radar_configs['data_type'])
                # else:
        ...
        if epoch % 2 == 0:
            from tools.test_eval import testval
            ap_toal,ar_toal = testval(test_model=rodnet.eval(),model_name=status_dict['model_name'])
            with open(train_log_name, 'a+') as f_log:
                    f_log.write('epoch %2d, iter %4d: AP: %.4f (%.4f) | AR: %.4f \n' %
                                (epoch + 1, iter + 1, ap_toal, ar_toal))

            writer.add_scalar('AP/ap_toal', ap_toal, iter_count)
            writer.add_scalar('AR/ar_toal', ar_toal, iter_count)
